...
class ReliefPetal {
  ...
  reset(first = false) {
    ...
    this.d = 0;
    this.dMax = random(this.bs * 0.18, this.bs * 0.32);
    this.col = random(this.palette.filter(c => c !== this.backgroundColor));
    this.fr = random(10284701987);
    this.growSpeed = random(0.00052, 0.0009);
    const firstDelay = this.index * random(55, 95) + random(0, 40);
    this.t = first ? -int(firstDelay) : -int(random(80, 420));
    this.state = 'grow';
    this.soundStarted = false;
    this.holdFrames = int(random(180, 420));
    this.holdCounter = 0;
    this.fadeSpeed = random(4, 8);
    this.opacity = 255;
  }
  ...
  move() {
    const n = noise(frameCount * 0.1 * this.bs, this.x, this.y);
    this.d += max(this.bs * 0.00026, n * this.bs * this.growSpeed);
  }
