...
function drawAnxietyCore(cx, cy, minDim) {
  ...
  const hotR = minDim * (0.11 + state.coreEnergy * 0.08) * randomPulse * microJitter;
  ...
  for (let i = 0; i < 18; i++) {
    ...
    let sparkle = random([[255, 24, 46], [255, 0, 128], [205, 0, 255], [255, 210, 240]]);
...
function drawAnxietyStrings(cx, cy, minDim) {
  ...
  for (let k = 0; k < 5; k++) {
    ...
    let hueType = k % 2 === 0 ? [255, 0, 70] : [185, 0, 255];
...
function drawAnxietyNumbers(cx, cy) {
  ...
  for (let d of numbersCloud) {
    ...
    let col = d.mode === "number"
      ? random([[255, 24, 46], [255, 0, 90], [255, 0, 150], [205, 0, 255]])
      : random([[255, 160, 220], [255, 120, 180], [230, 100, 255], [255, 205, 235]]);
...
function scribbleColor(kind) {
  ...
  if (kind === "magenta") return [255, 0, 145];
  if (kind === "violet") return [175, 0, 255];
  return [255, 24, 46];
}
