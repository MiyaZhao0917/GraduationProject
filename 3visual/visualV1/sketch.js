let phase = 0;              // 0 = anxiety, 1 = relief
...
class PanicDigit {
  ...
  reset() {
    ...
    this.char = random(DIGIT_SET.split(''));
  ...
  update(alphaFactor) {
    ...
    if (this.timer <= 0) {
      this.char = random(DIGIT_SET.split(''));
      this.size = constrain(this.size + random(-10, 10), 10, 52);
      this.timer = floor(random(2, 10));
    }

    if (random() < 0.015) {
      this.alpha = random(60, 255);
      this.char = str(floor(random(0, 10)));
    }
...
function mousePressed() {
  // clicking to switch between two stages, facilitating the preview of the projection effect
  phase = phase === 0 ? 1 : 0;
}
