...
const PLATFORM_WORDS = [
  ...
  'sent', 'read', 'active now', 'viewed', 'processing', 'refreshing', 'new post',
  'suggested for you', 'because you watched', 'posted just now', '5 new stories',
  'mutuals viewed this', 'updated now', '1 unread'
];
...
function drawStage1FeedGhosts() {
  ...
  stage1GhostOffset += deltaTime * 0.000025;

  for (let lane = 0; lane < laneCount; lane++) {
    const x = lane * laneW + laneW * 0.5;
    const baseW = laneW * 0.78;
    for (let i = -1; i < 7; i++) {
      const y = ((i * 128 + (frameCount * 0.55 + lane * 47)) % (height + 180)) - 90;
      const h = 88 + ((i + lane) % 3) * 22;
      const alpha = 18 + sin(frameCount * 0.01 + i * 0.8 + lane * 1.7) * 6;
      noStroke();
      fill(255, 255, 255, alpha * 0.05);
      rect(x, y, baseW, h, 18);
      fill(255, 255, 255, alpha * 0.12);
      circle(x - baseW * 0.38, y - h * 0.25, 18);
      rect(x - baseW * 0.08, y - h * 0.22, baseW * 0.36, 7, 3);
      rect(x - baseW * 0.04, y - h * 0.08, baseW * 0.46, 5, 3);
      rect(x - baseW * 0.08, y + h * 0.03, baseW * 0.54, 5, 3);
      if ((i + lane) % 2 === 0) {
        fill(255, 102, 84, alpha * 0.11);
        rect(x, y + h * 0.17, baseW * 0.78, h * 0.28, 12);
      }
    }
  }
  pop();
...
function updateStage1Elements() {
  ...
  if (stage1SpawnTimer <= 0) {
    spawnStage1Burst();
    stage1SpawnTimer = random(0.06, 0.18);
  }
...
function spawnStage1Burst() {
  const burstCount = floor(random(2, 6));
  for (let i = 0; i < burstCount; i++) {
    const r = random();
    if (r < 0.34) spawnFeedCard();
    else if (r < 0.58) spawnBadgeChip();
    else if (r < 0.77) spawnTypingBubble();
    else if (r < 0.9) spawnStoryStrip();
    else spawnMetricFlash();
  }
}
...
function spawnMetricFlash() {
  const ttl = random(1.0, 2.2);
  stage1Elements.push({
    type: 'metric',
    x: random(width * 0.08, width * 0.92),
    y: random(height * 0.1, height * 0.9),
    value: `${floor(random(2, 999))}`,
    text: random(['views', 'likes', 'watching', 'unread', 'shares']),
    ttl, life: ttl,
    vx: random(-16, 16),
    vy: random(-16, 8)
  });
...
function drawStage2() {
  ...
  if (phase2SpawnTimer <= 0) {
    spawnPhase2Word();
    phase2SpawnTimer = random(0.07, 0.22);
  }
...
function drawStage2Background() {
  ...
  for (let i = 1; i < laneCount; i++) {
    fill(255, 255, 255, 8);
    rect(i * laneW, height * 0.5, 1.2, height);
  }
  pop();
}

function getInterpretationWeights() {
  if (phase2Age < 4) return { platform: 0.72, judgement: 0.22, self: 0.06 };
  if (phase2Age < 9) return { platform: 0.4, judgement: 0.36, self: 0.24 };
  return { platform: 0.18, judgement: 0.34, self: 0.48 };
}
...
function spawnPhase2Word() {
  ...
  phase2Words.push(new Phase2Word(card.x + random(-card.w * 0.22, card.w * 0.22), card.y + random(-card.h * 0.1, card.h * 0.1), pick.text, pick.kind));
}
...
class Phase2Card {
  ...
  reset(first = false) {
    ...
    this.sub = random(['just now', 'seen', 'active', 'new', 'updated']);
    this.metric = `${floor(random(1, 99))}`;
  ...
  display() {
    ...
    if (this.kind !== 'comment') {
      noStroke();
      fill(255, 72, 78, 130);
      rect(this.x + this.w * 0.28, this.y + this.h * 0.26, 34, 18, 9);
      fill(255, 242, 238, 180);
      textAlign(CENTER, CENTER);
      textSize(10);
...
class Phase2Word {
  constructor(x, y, text, kind) {
    ...
    this.vy = random(-26, -12);
    this.life = random(1.0, 2.2);
    this.ttl = this.life;
  ...
  display() {
    ...
    if (random() < 0.14) {
      fill(255, 255, 255, 26 * a);
      text(this.text, this.x + random(-4, 4), this.y + random(-4, 4));
    }
    pop();
  }
}
