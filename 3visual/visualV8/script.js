...
function drawStage1FeedGhostsBalanced() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    strokeWeight(1);
    // broken top/bottom edges only: avoids the chat-card feeling in anxiety 2
...
function updateStage1Elements() {
  ...
  if (stage1SpawnTimer <= 0) {
    spawnStage1Burst();
    // slightly slower than V7.5 so the social cues remain legible in exhibition space
...
function spawnStage1Burst() {
  ...
  // re-introduced V5 social-media cues, but reduced into temporary traces/fragments
...
function drawStage1CardTrace(e, a, p) {
  // anixety1 uses feed shards, not chat bubbles. it should feel like platform residue caught by the refresh tide, clearly different from the attack cards in anxiety2
  push();
  translate(p.x, p.y);
  blendMode(SCREEN);
  rectMode(CENTER);
  ...
  noFill();
  // broken, angular edges: more like feed UI debris than a chat bubble
...
function drawStage1TypingTrace(e, a, p) {
  // a system-status ticker rather than a message bubble, so it doesn't compete with the Holzer-like attack text/cards in anxiety 2
  push();
  translate(p.x, p.y);
  blendMode(SCREEN);
  rectMode(CENTER);
