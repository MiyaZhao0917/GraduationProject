...
function draw() {
  ...
  if (transitionState) {
    ...
    // the transition animation itself is the whole image: background + source elements + target structure
...
function collectTransitionAnchors(sourcePhase, desiredCount = 120) {
  ...
  if (sourcePhase === 0) {
    ...
    // reserve the moving refresh seams as real source material for the a1-a2 fold
    for (const tide of stage1Tides) {
      anchors.push({
        x: width * 0.5,
        y: tide.y,
...
function seedStage1ForTransitionTarget() {
  // anxiety1 normally grows its social traces over time. for a morph target i need a real first frame
  // so the last transition frame can land on actual notification elements instead of empty space
...
// anxiety1-anxiety2: gaze, cards, and words dissolve into water/light/bloom
...
function drawGazeToReliefTransform(state, p) {
  ...
  push();
  noStroke();
  // hold the red-black pressure slightly longer, then let the relief wash arrive as an opening
  const hold = 1 - smoother(constrain((p - 0.18) / 0.64, 0, 1));
  fill(10, 0, 7, 76 * hold);
  rect(width * 0.5, height * 0.5, width, height);
  const open = easeOutCubic(constrain((p - 0.34) / 0.52, 0, 1));
  ...
  drawA2RevelationCompression(state, p);
  drawA2EyeCollapsingIntoRipple(state, p);
  drawA2RevelationOpening(state, p);
...
function drawA2EyeCollapsingIntoRipple(state, p) {
  ...
  const qClose = smoother(constrain(p / 0.36, 0, 1));
  ...
  // closed eye becomes an aperture: a brief bright opening before it settles into real relief ripples
  if (burst > 0) {
...
function commitPhaseAfterTransition() {
  ...
  // target phase has already been generated at transition start and used as the morph destination
...
function drawIntegratedPhaseMorph(state, p) {
  ...
  drawMorphingSubstrate(state, p, q);
  ...
  // fade in the actual pre-generated target stage elements, not a separate decorative arrival scene
...
function morphPaletteForKey(key) {
  if (key === '0-1') {
    return {
      // anxiety1 exact base - anixety2 exact base
  ...
  if (key === '1-2') {
    return {
      // anixety2 exact base - relief exact base
...
function drawMorphingSubstrate(state, p, q) {
  ...
  if (state.key === '0-1') {
    ...
    // gaze field first tightens toward the centre, then opens into slow water lines
    const pinch = sin(constrain((q - 0.08) / 0.48, 0, 1) * PI);
    const open = smoother(constrain((q - 0.34) / 0.66, 0, 1));
    const cx = width * 0.5;
    const cy = height * 0.52;
    for (let i = 0; i < 18; i++) {
      const yBase = map(i, 0, 17, height * 0.02, height * 0.98);
      stroke(lerp(255, 202, q), lerp(38, 238, q), lerp(70, 230, q), lerp(36 + 30 * pinch, 44, open));
      strokeWeight(lerp(1.5 + pinch * 0.9, 2.2, open));
      beginShape();
      for (let x = -80; x <= width + 80; x += 22) {
        const hard = yBase + sin(x * 0.022 + i + frameCount * 0.035) * 24;
        const squeezedX = lerp(x, cx, pinch * 0.45);
        const squeezedY = lerp(hard, cy, pinch * 0.62);
        const soft = yBase + sin(x * 0.005 + i + frameCount * 0.007) * 9;
        curveVertex(lerp(squeezedX, x, open), lerp(squeezedY, soft, open));
      }
      endShape();
    }
  } else if (state.key === '2-0') {
...
function drawGeneratedTargetPhasePreview(state, p) {
  ...
  const start = state.key === '1-2' ? 0.54 : 0.68;
  const span = state.key === '1-2' ? 0.46 : 0.32;
...
function setPhase(nextPhase) {
  ...
  // avoids the next background appearing too early
  if (transitionState) return;
