// there are still codes about servos in this file though I did not use them at this stage
// these codes don not affect visual and interaction

let phase = 0; // 0 = anxiety1, 1 = anxiety2, 2 = relief

// -----relief auto-return timer -----
// after the relief stage is fully entered, wait 2 minutes, then use the same phase-switching logic as pressing key 1 to return to anxiety1
const RELIEF_AUTO_RETURN_DELAY_MS = 2 * 60 * 1000;
let reliefAutoReturnTimer = null;

function clearReliefAutoReturnTimer() {
  if (reliefAutoReturnTimer !== null) {
    clearTimeout(reliefAutoReturnTimer);
    reliefAutoReturnTimer = null;
  }
}

function startReliefAutoReturnTimer() {
  clearReliefAutoReturnTimer();
  reliefAutoReturnTimer = setTimeout(() => {
    reliefAutoReturnTimer = null;

    // only auto-return if the work is still calmly sitting in relief
    // if the audience / Arduino / keyboard has already moved to another state, do nothing
    if (phase === 2 && !transitionState) {
      setPhase(0);
    }
  }, RELIEF_AUTO_RETURN_DELAY_MS);
}
...
// send simple mode commands to Arduino
...
function sendServoMode(mode) {
  ...
  // SERVO:A1   = anxiety 1 subtle occasional movement
  // SERVO:A2   = anxiety 2 stronger movement
...
function syncServoToCurrentVisualState() {
  ...
  if (transitionState) {
    // during anixety2-relief transition, servo should already be still
    if (transitionState.from === 1 && transitionState.to === 2) sendServoMode('STOP');
    // during anxiety1-anxiety2 transition, keep anxiety1 subtle mode until the animation commits
...
// ----- phase transitions: element transformation layer -----
// this layer sits above the three finished stages
// it does not replace the stage visuals
// it carries fragments from the previous stage and lets them morph into the visual language of the next stage
let transitionState = null;

// offscreen buffer used only for the anxiety 2-relief blur layers
// this replaces Canvas 2D drawingContext.filter with p5's built-in filter(BLUR)
let a2BlurLayer = null;


// ----- sound layer -----
// audio is intentionally added as a separate layer
const SOUND_PATHS = {
  pop: 'pop.mp3',
...
function setupSoundLayer() {
  ...
  // first click/key press, or the Connect Arduino button, unlocks the sound layer for later sensor use
...
function unlockSoundLayer() {
  ...
  // quietly prime the audio objects so later Arduino-triggered transitions can play
...
function stopStage1Sounds() {
  // stop only the anxiety1 element sounds
  // do not stop transfer1, so the anxiety1-anxiety2 transition audio can keep playing normally
  stopSoundPool('pop');
...
function stopStage2Sounds() {
  // stop the anxiety2 background layer immediately when leaving phase 2
  // especially before the anxiety2-relief transition sound starts
  stopSoundPool('radio');
...
// ----- anxiety2 audience interaction prompt -----
// the prompt is drawn inside the anxiety 2 canvas layer only
const STAGE2_PROMPT_VAGUE_START = 15;
const STAGE2_PROMPT_CLEAR_START = 25;
...
function ensureA2BlurLayer() {
  if (!a2BlurLayer || a2BlurLayer.width !== width || a2BlurLayer.height !== height) {
    a2BlurLayer = createGraphics(width, height);
    a2BlurLayer.pixelDensity(1);
  }
  return a2BlurLayer;
}

function drawP5FilteredBlurLayer(drawIntoLayer, blurAmount, alphaValue, useScreenBlend = false) {
  if (alphaValue <= 0.001) return;

  const layer = ensureA2BlurLayer();
  layer.clear();
  layer.push();
  ...
  push();
  if (useScreenBlend) blendMode(SCREEN);
  tint(255, 255 * constrain(alphaValue, 0, 1));
  imageMode(CORNER);
  image(layer, 0, 0, width, height);
  noTint();
  if (useScreenBlend) blendMode(BLEND);
  pop();
}
...
function initStage1() {
  ...
  // each tide is a moving distorted band
  // when it touches front-layer signals, the signal flashes as if it has been refreshed away
  stage1Tides = [];
  const tideCount = 4;
...
function showUI(seconds = 4) {
  // UI visibility is controlled only by the H key
  // other actions can still call showUI(), but they can't force the panel back on
  applyUIVisibility();
}
...
function draw() {
  ...
  if (transitionState) {
    ...
    // during transition, do not draw the old stage underneath and do not draw the next stage early
    // the transition scene itself is the whole image
...
// -----anxiety1: balanced refresh tide + social-media traces -----
function drawStage1() {
  updateStage1Tides();
...
function drawStage1Background() {
  ...
  g.addColorStop(0, '#050107');
  g.addColorStop(0.32, '#14020d');
  g.addColorStop(0.62, '#090208');
  ...
  // subtle stretched substrate: background lines bend near the refresh tides, so the whole screen feels pulled rather than simply scanned
  push();
  blendMode(SCREEN);
  strokeWeight(1);
...
function drawRefreshTideBody(tide, front = false) {
  ...
  // these lines create the visual feeling that the canvas is being vertically stretched and re-sampled
  noFill();
  for (let k = -5; k <= 5; k++) {
    const off = k * 9;
...
function drawStage1FeedGhostsBalanced() {
  // very faint platform ghosts in the background: not chat boxes or charts
  // they read more like feed residue / notification trails caught inside the refresh tide
  push();
  blendMode(SCREEN);
  rectMode(CENTER);
  noFill();
...
function updateStage1Elements() {
  ...
  if (stage1SpawnTimer <= 0) {
    spawnStage1Burst();
    // the social cues remain legible in exhibition space
...
function spawnStage1Burst() {
  ...
  // reduce bait dots so they act as occasional temptations rather than the main visual layer
  let dotNum = 0;
  ...
  // social-media cues in temporary traces/fragments
  // only a few are visible at once, so the stage stays abstract but still readable
...
function drawBaitGlyph(e, a, p) {
  ...
  noStroke();
  fill(e.hot || p.glow > 0.4 ? color(255, 205, 105, 250 * a) : color(255, 52, 62, 220 * a));
  text(e.text, 0, 0);
  fill(255, 240, 224, (88 + p.glow * 175) * a);
...
function drawStage1TypingTrace(e, a, p) {
  ...
  fill(255, 232, 220, (170 + flash * 80) * a);
  textAlign(LEFT, CENTER);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    fill(255, 142 + flash * 80, 108 + flash * 80, (170 + flash * 75) * a * pulse);
...
function drawStage2() {
  ...
  drawStage2InteractionPrompt();
  drawStage2ChromaticBurn();
  drawStage2FeedMask();
}

function drawStage2InteractionPrompt() {
  if (phase2Age < STAGE2_PROMPT_VAGUE_START) return;

  const isClearPrompt = phase2Age >= STAGE2_PROMPT_CLEAR_START;
  const label = isClearPrompt ? 'PRESS THE BUTTON WHEN YOU ARE READY' : 'there is a way out';

  const ageInPrompt = phase2Age - (isClearPrompt ? STAGE2_PROMPT_CLEAR_START : STAGE2_PROMPT_VAGUE_START);
  const appear = easeInOutCubic(constrain(ageInPrompt / 1.2, 0, 1));
  const unstable = isClearPrompt ? 0.18 : 0.72;
  const pulse = 0.5 + 0.5 * sin(frameCount * (isClearPrompt ? 0.045 : 0.082));
  const glitch = noise(phase2Age * 0.9, frameCount * 0.035);

  const x = width * 0.5 + sin(frameCount * 0.034) * unstable * 5 + (glitch - 0.5) * unstable * 8;
  const y = height * 0.68 + cos(frameCount * 0.027) * unstable * 4;
  const textSizeValue = isClearPrompt ? constrain(width * 0.014, 15, 21) : constrain(width * 0.012, 12, 17);
  const cardW = min(width * 0.76, max(isClearPrompt ? 430 : 270, label.length * textSizeValue * 0.74));
  const cardH = isClearPrompt ? 66 : 52;
  const baseAlpha = appear * (isClearPrompt ? 0.92 : 0.46 + pulse * 0.16);
  ...
  // keep the prompt visually inside anxiety2: dark card, red/purple edge, slight system-like scan
  drawingContext.shadowBlur = isClearPrompt ? 24 : 14;
  drawingContext.shadowColor = `rgba(255, 28, 58, ${0.22 * baseAlpha})`;

  noStroke();
  fill(10, 0, 8, 118 * baseAlpha);
  rect(x, y, cardW, cardH, 14);

  strokeWeight(isClearPrompt ? 1.45 : 1);
  stroke(255, 38, 66, (isClearPrompt ? 132 : 70) * baseAlpha);
  noFill();
  rect(x, y, cardW, cardH, 14);

  stroke(138, 0, 160, (isClearPrompt ? 78 : 48) * baseAlpha);
  line(x - cardW * 0.46, y - cardH * 0.22, x + cardW * 0.46, y - cardH * 0.22);
  line(x - cardW * 0.46, y + cardH * 0.22, x + cardW * 0.46, y + cardH * 0.22);

  drawingContext.shadowBlur = isClearPrompt ? 18 : 10;
  drawingContext.shadowColor = `rgba(255, 220, 224, ${0.28 * baseAlpha})`;
  noStroke();
  fill(255, 226, 230, 220 * baseAlpha);
  textSize(textSizeValue);
  text(label, x, y + (isClearPrompt ? 1 : 0));

  // during the vague stage, small dark interruptions make the sentence feel half-found in the noise
  if (!isClearPrompt) {
    noStroke();
    fill(8, 0, 7, 72 * baseAlpha);
    const cutCount = 2 + floor(glitch * 3);
    for (let i = 0; i < cutCount; i++) {
      const cutX = x - cardW * 0.36 + noise(i * 4.9, phase2Age * 0.8) * cardW * 0.72;
      const cutY = y - 9 + noise(i * 7.1, frameCount * 0.02) * 18;
      rect(cutX, cutY, 22 + noise(i * 2.3, phase2Age) * 34, 2 + noise(i * 3.7, frameCount * 0.01) * 3, 1.5);
    }
  }

  // a thin moving scan line ties the prompt to the existing anxiety2 scanning language
  const scanY = y - cardH * 0.35 + ((frameCount * 1.4) % (cardH * 0.7));
  stroke(255, 68, 80, 60 * baseAlpha);
  strokeWeight(1);
  line(x - cardW * 0.44, scanY, x + cardW * 0.44, scanY);
...
function drawStage2Background() {
  const g = drawingContext.createLinearGradient(0, 0, 0, height);
  g.addColorStop(0, '#050009');
  g.addColorStop(0.28, '#190018');
  g.addColorStop(0.58, '#2a020a');
  g.addColorStop(0.8, '#100006');
  g.addColorStop(1, '#010102');
  drawingContext.fillStyle = g;
  drawingContext.fillRect(0, 0, width, height);
...
function phase2KindColor(kind, alpha) {
  if (kind === 'platform') return color(255, 118, 36, alpha);
  if (kind === 'judgement') return color(255, 24, 62, alpha);
  if (kind === 'self') return color(255, 238, 244, alpha);
  return color(255, 92, 132, alpha);
}
...
class Phase2Word {
  ...
  display() {
    ...
    if (this.kind === 'platform') fill(255, 120, 42, 165 * a);
    else if (this.kind === 'judgement') fill(255, 24, 60, 245 * a);
    else if (this.kind === 'self') fill(255, 240, 246, 255 * a);
    else fill(255, 92, 132, 95 * a);
...
function drawReliefBackground() {
  const g = drawingContext.createLinearGradient(0, 0, 0, height);
  g.addColorStop(0, '#061d24');
  g.addColorStop(0.34, '#0c333b');
  g.addColorStop(0.7, '#14505a');
  g.addColorStop(1, '#08242d');
  drawingContext.fillStyle = g;
  drawingContext.fillRect(0, 0, width, height);

  const glow1 = drawingContext.createRadialGradient(width * 0.25, height * 0.25, 0, width * 0.25, height * 0.25, width * 0.45);
  glow1.addColorStop(0, 'rgba(226,222,168,0.22)');
  ...
  glow2.addColorStop(0, 'rgba(120,255,218,0.20)');
...
function drawFlowingLight() {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (i % 3 === 0) stroke(150, 255, 226, 86 - i * 2.5);
    else if (i % 3 === 1) stroke(120, 205, 255, 74 - i * 2.2);
    else stroke(255, 215, 165, 60 - i * 1.6);
...
class ReliefPetal {
  constructor(index) {
    ...
    this.palette = ['#c7d8ce', '#9ecfc1', '#8fb6c9', '#9f9ab0', '#b5a88d', '#d8b3b8'];
    this.backgroundColor = '#061d24';
...
function transitionDuration(from, to) {
  ...
  if (key === '1-2') return 5.15; // stronger revelation: gaze compresses, opens, then lands into relief
...
function phase2TargetKindColor(kind, alphaValue = 180) {
  if (kind === 'platform') return color(255, 118, 36, alphaValue);
  if (kind === 'judgement') return color(255, 24, 62, alphaValue);
  if (kind === 'self') return color(255, 238, 244, alphaValue);
  return color(255, 92, 132, alphaValue);
}
...
function collectTransitionAnchors(sourcePhase, desiredCount = 120) {
  ...
  if (sourcePhase === 0) {
    ...
    // preserve the moving refresh seams as real source material for the anxiety1-anxiety2 fold.
    for (const tide of stage1Tides) {
      anchors.push({
        x: width * 0.5,
        y: tide.y,
...
function createPhaseTransform(from, to) {
  ...
  // pre-generate the next stage once, before the transition is drawn
  // the morph targets below are taken from this exact generated stage, and commit will not randomise it again
  initPhase(to);
...
function prepareTargetPhaseForTransition(to) {
  ...
  if (to === 2) {
    // relief needs one priming update because several objects calculate their visible x/y/size in update()
...
function seedStage1ForTransitionTarget() {
  // anxiety1 normally grows its social traces over time
  // for a morph target I need a real first frame
  // so the last transition frame can land on actual notification elements instead of empty spac
...
// ----- anxiety1-anxiety2 (0-1): individual feed signals are misread and organised into gaze -----
...
function drawFeedToGazeTransform(state, p) {
  ...
  drawTransitionLabel(state, p, color(255, 72, 92));
}
...
function drawA1RefreshBandsFoldingIntoEye(state, p, cx, cy, eyeW, eyeH) {
  ...
  for (let i = 0; i < state.seams.length; i++) {
    ...
    stroke(255, lerp(38, 42, k), lerp(54, 76, k), a);
...
// ----- anxiety2-relief (1-2): gaze, cards, and words dissolve into water/light/bloom -----
function buildGazeToReliefTransform(state, anchors) {
  const centerX = width * 0.5;
  const centerY = height * 0.52;

  // anxiety2-relief is the conceptual turning point, so it gets a stronger dramaturgy
  // the judgement field is pulled into one tense point, then opens into the exact relief targets
  state.revelation = {
    cx: centerX,
    cy: centerY,
    preludeEnd: 0.37,
...
function drawA2SourceStageDissolve(state, p) {
  ...
  // use the exact last anxiety2 frame as the first transition image
  // this removes the blank jump and makes the transition begin from the same visual state
  // blur is applied with p5's built-in filter(BLUR) on an offscreen buffer instead of drawingContext.filter to avoid jag
  if (state.sourceSnapshot) {
    drawP5FilteredBlurLayer((layer) => {
      layer.image(state.sourceSnapshot, 0, 0, width, height);
    }, blurAmount, sourceFade, false);
  } else {
    push();
    drawingContext.globalAlpha *= sourceFade;
    drawStage2SourcePreviewNoUpdate();
    pop();
  }

  // a soft copy of the live elements stays over the blurred snapshot for the first half
  // so words and boxes feel like they are dissolving rather than being replaced by fragments
  if (t < 0.72) {
    const dissolveAlpha = (1 - smoother(constrain((t - 0.22) / 0.50, 0, 1))) * 0.48;
    drawA2SourceBlurDissolve(state, t * 0.72, dissolveAlpha);
  }
...
function drawA2SourceBlurDissolve(state, p, layerAlpha = 1) {
  // a short prelude for anxiety2-relief: keep the actual anxiety2 source material visible
  // then let the words/cards blur away while the eye closes before the stronger collapse continues
  const prelude = constrain(p / 0.38, 0, 1);
  const fade = 1 - smoother(prelude);
  if (fade <= 0.002 || layerAlpha <= 0.001) return;
  ...
  drawP5FilteredBlurLayer((layer) => {
    ...
    for (let i = 0; i < anchors.length; i++) {
      const a = anchors[i];
      if (a.kind === 'phase2Card') {
        drawA2SourceCardDissolving(layer, a, i, drift, fade);
      } else if (a.kind === 'phase2Word' || a.kind === 'phase2RingWord') {
        drawA2SourceWordDissolving(layer, a, i, drift, fade);
      }
    }

    layer.blendMode(BLEND);
  }, blurAmount, layerAlpha * fade, true);
}

function drawA2SourceCardDissolving(layer, a, index, drift, fade) {
  ...
  layer.push();
  layer.translate(x, y);
  layer.rotate(sin(seed + frameCount * 0.01) * 0.018 * soften);

  layer.noStroke();
  layer.fill(8, 0, 8, 82 * fade * localFlicker);
  layer.rect(0, 0, w, h, 10);

  layer.stroke(255, 44, 72, 74 * fade * localFlicker);
  layer.strokeWeight(lerp(1.0, 0.45, drift));
  layer.noFill();
  layer.rect(0, 0, w, h, 10);

  layer.noStroke();
  layer.textAlign(CENTER, CENTER);
  layer.textSize(lerp(12, 9, drift));
  const c = phase2TargetKindColor(a.textKind || 'judgement', 138 * fade * localFlicker);
  layer.fill(c);
  layer.text(a.text || randomStablePhase2Word(seed), 0, -h * 0.16, w * 0.82);

  layer.fill(255, 112, 78, 56 * fade * soften);
  layer.textSize(9);
  layer.text(a.metric || 'read', 0, h * 0.19, w * 0.82);
  layer.pop();
}

function drawA2SourceWordDissolving(layer, a, index, drift, fade) {
  ...
  layer.push();
  layer.translate(x + sin(frameCount * 0.025 + seed) * jitter, y + cos(frameCount * 0.02 + seed) * jitter);
  layer.rotate((a.rotate || a.angle || 0) * (1 - drift * 0.35));
  layer.textAlign(CENTER, CENTER);
  layer.textSize(s);

  if (a.kind === 'phase2RingWord') layer.rotate(HALF_PI);

  layer.fill(255, 0, 42, 34 * fade);
  layer.text(textValue, -2 - drift * 2, 1);
  layer.fill(70, 230, 255, 14 * fade);
  layer.text(textValue, 2 + drift * 2, -1);
  layer.fill(phase2TargetKindColor(a.textKind || 'noise', 150 * fade));
  layer.text(textValue, 0, 0);
  layer.pop();
}
...
function drawGazeToReliefTransform(state, p) {
  ...
  // the first part should still look like anxiety: only blur, dissolve and eye-closing happen here
  // the stronger collapse starts after the prelude, so there is no empty break between anxiety2 and the morph
  if (p < preludeEnd) {
  ...
  // fade the transition-only wash out at the very end so the last frame lands on the real relief scene
  const settleFade = 1 - smoother(constrain((collapseP - 0.78) / 0.22, 0, 1));
  const wash = drawingContext.createRadialGradient(cx, cy, 0, cx, cy, max(width, height) * 0.92);
  wash.addColorStop(0, `rgba(120,255,218,${(0.05 * q + 0.16 * open) * settleFade})`);
  wash.addColorStop(0.30, `rgba(80,170,210,${(0.035 * q + 0.08 * open) * settleFade})`);
  wash.addColorStop(0.62, `rgba(226,222,168,${0.018 * open * settleFade})`);
  wash.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawA2RevelationOpening(state, p) {
  ...
  const settleFade = 1 - smoother(constrain((p - 0.76) / 0.24, 0, 1));
  if ((burst <= 0.001 && open <= 0.001) || settleFade <= 0.001) return;
  ...
  glow.addColorStop(0, `rgba(150,255,226,${(0.18 * burst + 0.07 * open) * settleFade})`);
  glow.addColorStop(0.18, `rgba(105,220,205,${(0.14 * burst + 0.055 * open) * settleFade})`);
  glow.addColorStop(0.48, `rgba(80,165,210,${(0.07 * burst + 0.04 * open) * settleFade})`);
  ...
  for (let i = 0; i < 11; i++) {
    ...
    stroke(150, 255, 226, a * 0.72 * settleFade);
  ...
  for (let i = 0; i < 32; i++) {
    ...
    stroke(150, 255, 226, 18 * burst * settleFade * (0.5 + 0.5 * noise(rv.seed, i)));
    strokeWeight(0.8);
...
function drawA2AfterglowBreath(state, p) {
  ...
  const settleFade = 1 - smoother(constrain((p - 0.76) / 0.24, 0, 1));
  if (open <= 0.001 || settleFade <= 0.001) return;
  push();
  blendMode(SCREEN);
  noFill();
  const cx = width * 0.5;
  ...
  for (let i = 0; i < 6; i++) {
    ...
    stroke(150, 255, 226, (18 - i * 1.8) * open * settleFade);
    strokeWeight(1.1);
...
function drawA2EyeCollapsingIntoRipple(state, p) {
  ...
  // the eye has already closed during the prelude, so the collapse starts from a closed slit
  const qClose = 1;
  ...
  const settleFade = 1 - smoother(constrain((p - 0.82) / 0.18, 0, 1));
  ...
  // the closed eye becomes an aperture: a brief bright opening before it settles into real relief ripples
  if (burst > 0) {
    stroke(150, 255, 226, 58 * burst * settleFade);
    strokeWeight(2.0);
    ellipse(cx, cy, eyeH * (0.36 + burst * 0.18), eyeH * (0.10 + burst * 0.08));
    noStroke();
    fill(150, 255, 226, 42 * burst * settleFade);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const ringAlpha = max(0, a + burst * 18) * settleFade;
    if (ringAlpha <= 0.01) continue;
    stroke(lerp(255, 205, qRipple), lerp(45, 246, qRipple), lerp(72, 238, qRipple), ringAlpha);
...
// -----relief-anxiety1 (2-0): calm light contracts into notification/feed pressure -----
...
function drawReliefRingsCompressing(state, p) {
  ...
  for (const ring of state.rings) {
    ...
    const a = 42 * sin(lp * PI);
    stroke(lerp(165, 255, q), lerp(255, 28, q), lerp(230, 50, q), a);
    strokeWeight(lerp(2.2, 1.0, q));
    ellipse(ring.sx, ring.sy, r * 2, r * 1.4);
    if (q > 0.78) {
      noStroke();
      fill(255, 28, 45, 80 * (q - 0.78) / 0.22);
...
function drawReliefParticleToFeedSignal(item, p) {
  ...
  if (item.finalKind === 'wordToBadge' || item.finalKind === 'moteToChip') {
    ...
    for (let i = 0; i < dots; i++) {
      ...
      fill(255, 28 + i * 12, 50, 74 * signalA);
      circle(cos(a2) * d * 0.35, sin(a2) * d * 0.35, lerp(18, 5, q));
    }
  } else {
    noStroke();
    fill(255, 28, 50, 118 * signalA);
...
function drawFeedRefreshReappearing(state, p) {
  ...
  // red bait dots restart as a recognisable anxiety1 motif
  for (let i = 0; i < 10; i++) {
    const x = noise(state.seed, i * 13.7) * width;
    const y = noise(state.seed + 100, i * 9.1) * height;
    noStroke();
    fill(255, 28, 50, 42 * q * (0.4 + noise(i, frameCount * 0.04)));
...
function commitPhaseAfterTransition() {
  ...
  // the target phase has already been generated at transition start and used as the morph destination
  // do not call initPhase() here, otherwise the final morph frame and the next phase would diverge again
...
function drawIntegratedPhaseMorph(state, p) {
  ...
  // fade in the actual pre-generated target stage elements
  // this makes the end of the morph and the first frame of the next phase share the same objects
...
function morphPaletteForKey(key) {
  if (key === '0-1') {
    return {
      // anxiety1 exact base-anxiety 2 exact base
      topA: color(5, 1, 7), midA: color(20, 2, 13), botA: color(1, 1, 2),
      topB: color(5, 0, 9), midB: color(25, 0, 24), botB: color(1, 1, 2),
      glowA: color(255, 28, 50), glowB: color(150, 0, 95), glowC: color(255, 92, 12)
    };
  }
  if (key === '1-2') {
    return {
      ...
      // match the new relief dark background
      topB: color(6, 29, 36),
      midB: color(12, 51, 59),
      botB: color(8, 36, 45),

      // darker, less white glow
      glowA: color(120, 255, 218),
      glowB: color(80, 165, 205),
      glowC: color(226, 222, 168)
    };
  }
  if (key === '2-0') {
    return {
      // relief exact base - anxiety1 exact base
      topA: color(6, 29, 36), midA: color(12, 51, 59), botA: color(8, 36, 45),
      topB: color(5, 1, 7), midB: color(20, 2, 13), botB: color(1, 1, 2),
      glowA: color(255, 28, 45), glowB: color(42, 18, 60), glowC: color(88, 42, 54)
    };
  }
  return {
...
function drawMorphingBackground(state, q) {
  ...
  const glowStrength = state.key === '1-2' ? 0.12 + q * 0.045 : 0.18;
  ...
  g2.addColorStop(0, rgbaFromColor(pal.glowC, state.key === '1-2' ? 0.045 * (1 - q) + 0.065 * q : 0.10 + 0.08 * q));
...
function drawMorphingSubstrate(state, p, q) {
  ...
  if (state.key === '0-1') {
    // old refresh-tide substrate stops being horizontal and bends into an eye orbit
    const cx = width * 0.5;
    const cy = height * 0.42;
    const eyeW = min(width * 0.84, height * 1.35);
    const eyeH = eyeW * 0.31;
    for (let i = 0; i < 16; i++) {
      ...
      stroke(lerp(255, 255, q), lerp(38, 42, q), lerp(54, 76, q), 24 + 34 * q);
    ...
    // relief breathing lines flatten and speed up into refresh bands
    for (let i = 0; i < 14; i++) {
      const y = map(i, 0, 13, height * 0.04, height * 0.98) + q * height * 0.08;
      stroke(lerp(165, 255, q), lerp(255, 28, q), lerp(230, 50, q), 20 + 44 * q);
...
function drawMorphingSurfaceGrain(state, p) {
  ...
  const settleFade = state.key === '1-2' ? 1 - smoother(constrain((p - 0.86) / 0.14, 0, 1)) : 1;
  for (let i = 0; i < amount; i++) {
    const n = noise(state.seed, i * 0.17, frameCount * 0.012);
    if (state.key === '1-2') fill(215, 246, 238, (4 + 10 * q) * settleFade);
...
function drawGeneratedTargetPhasePreview(state, p) {
  // for anxiety2-relief, the target relief scene should begin to become visible during the revelation,
  // but it still uses the same generated relief objects that the morph lands on
  ...
  if (state.key === '1-2' && state.to === 2) {
    // keep the large relief light spots moving during the final preview
    // without this, the transition preview shows the generated relief objects frozen at transition-start time,
    // then the committed relief frame recalculates their positions from the current frameCount and visibly jumps
    syncReliefSoftLightMotionForTransitionPreview();

    const backgroundSettle = easeInOutCubic(constrain((p - 0.82) / 0.18, 0, 1));
    if (backgroundSettle > 0.001) {
      drawingContext.save();
      drawingContext.globalAlpha *= backgroundSettle;
      drawReliefBackground();
    ...
    drawReliefTargetPreviewNoUpdate();
    drawingContext.restore();
    return;
  }
...
function syncReliefSoftLightMotionForTransitionPreview() {
  // only sync side-effect-free motion for the anixety2-relief landing
  // do not update petals/ripples, because their update methods can trigger sounds or spawn new rings
  for (const blob of reliefBlobs) blob.update();
}
...
function drawTransitionVeil() {
  // a very light grain that helps projection hide banding
  // intentionally not a transition itself
  push();
  noStroke();
  for (let i = 0; i < 26; i++) {
    fill(255, 3);
...
function setPhase(nextPhase) {
  // during a transition, keep the already-visible phase stable
  // this avoids the next background appearing too early
  if (transitionState) return;
  ...
  if (nextPhase === phase) {
    ...
    if (phase === 2) startReliefAutoReturnTimer();
    else clearReliefAutoReturnTimer();
  ...
  if (nextPhase === 0) {
    // when returning to anxiety 1, clear every possible sound tail before the transition starts
    stopAllSounds();
  } else {
...
function mousePressed() {
  unlockSoundLayer();
  // mouse click no longer advances stages
  // keep projection interaction driven by Arduino sensors
  showUI();
}
