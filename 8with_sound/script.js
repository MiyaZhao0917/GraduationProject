...
// so it does not change any visual timing, position, colour, movement, or element lifecycle
const SOUND_PATHS = {
  pop: 'pop.mp3',
...
function setupSoundLayer() {
  ...
  // the first click/key press, or the Connect Arduino button, unlocks the sound layer for later sensor use
...
function stopStage1Sounds() {
  // stop only the anxiety1 element sounds. do not stop transfer1,
  // so the anxiety1-anxiety2 transition audio can keep playing normally
  stopSoundPool('pop');
...
function playReliefFlowerSound() {
  ...
  playSound(picked, { chance: 0.78, volume: random(0.24, 0.38) });
  return true;
}
...
function setPhase(nextPhase) {
  ...
  if (nextPhase === 0) {
    // when returning to anxiety1, clear every possible sound tail before the transition starts
    stopAllSounds();
  } else {
