...
function drawA2SourceStageDissolve(state, p) {
  ...
  // use the exact last anxiety 2 frame as the first transition image
...
function drawA2SourceBlurDissolve(state, p) {
  // a short prelude for anxiety2-relief: keep the actual Anxiety 2 source material visible,
...
function drawGazeToReliefTransform(state, p) {
  ...
  // first part should still look like anxiety2: only blur, dissolve and eye-closing happen here
  // stronger collapse starts after the prelude, so there is no empty break between anxiety2 and the morph
  if (p < preludeEnd) {
